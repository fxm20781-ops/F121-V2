...
def train_models_with_real_data():
    excel_filename = "F121_Data.xlsx" # 👈 請確保 GitHub 上的檔名完全叫這個
    
    if not os.path.exists(excel_filename):
        st.error(f"❌ 找不到資料檔 {excel_filename}，請確認是否有上傳到 GitHub。")
        st.stop()
        
    # 【核心修正】明確指定讀取 "Sheet1" 分頁，並跳過第 1 行的 Tag 代號
    # 如果您的數據分頁叫別的名字（例如 Sheet2），請把下面改成 sheet_name="Sheet2"
    df = pd.read_excel(excel_filename, sheet_name="Sheet1", skiprows=[1]) 
    
    # 清理欄位名稱（移除換行符號與前後多餘空格）
    ...
    if len(df_clean) == 0:
        st.error("❌ Excel 數據解析後為空，請確認填寫的 sheet_name 是否正確，且內部包含真實數據。")
        st.stop()
...
with st.spinner("🚀 正在精準讀取 Sheet1 數據並訓練 AI 模型..."):
    model_ng, model_c122, bounds = train_models_with_real_data()

# --- 3. 側邊欄：不可控變數輸入 ---
st.sidebar.header("📋 當前不可控排程設定")
input_dt = st.sidebar.slider("DT operation (稼率)", min_value=bounds['DT operation'][0], max_value=bounds['DT operation'][1], value=(bounds['DT operation'][0]+bounds['DT operation'][1])/2, step=0.01)
input_c141 = st.sidebar.slider("C141 operation (稼率)", min_value=bounds['C141 operation'][0], max_value=bounds['C141 operation'][1], value=(bounds['C141 operation'][0]+bounds['C141 operation'][1])/2, step=0.01)
...
res_col1, res_col2 = st.columns(2)
res_col1.metric(label="🏃 手動設定下的預估天然氣消耗 (Y)", value=f"{manual_y:.2f}")
res_col2.metric(label="🌡️ 手動設定下的預估 C122 塔底溫度", value=f"{manual_c122:.2f} °C")mport streamlit as st
import pandas as pd
import numpy as np
from lightgbm import LGBMRegressor
from scipy.optimize import minimize
